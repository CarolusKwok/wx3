...
load_file = function(data,
  ...
  if(worker <= 0){
    ...
    data_download = dplyr::filter(.data = wx3:::load_file.format(data = data), !Exist)
    ...
    } else if (nrow(data_download) > 0){
      temp_worker = ifelse(nrow(data_download) > worker * 2, worker, ceiling((nrow(data_download)/3)))
    ...
    for(i in seq_len(attempt)){
      if(nrow(data_download) > 0){
        cli::cli_text("{i} Attempt | {nrow(data_download)} File | {future::nbrOfWorkers()} Worker:")
        list_attempt[[i]] = nrow(data_download)
        wx3:::load_file.engine(data_download = data_download,
                               tmp_dir = tmp_dir,
                               threshold = threshold,
                               ... = ...)
        data_download = wx3:::load_file.format(data_download)
        cli::cli_text("{sum(data_download$Exist)} Success | {sum(!data_download$Exist)} Fail")
