#' @title System tool: download filesets using sequential computation
#'
#' @description
#' Similar to `wx3::load_fileset`, but forced to have sequential computation.
#'
#' @param data `dataframe` with character columns of `URL`, `DIR`, and `Info`.
...
#' @return
#' @keywords internal
load_fileset.SEQ = function(data, attempt, threshold, list_fail, title, ...){
  #Start ####
  ...
  for(i in seq_len(attempt)){
    if(nrow(data_download) > 0){
      ...
      wx3:::load_fileset.SEQengine(data_download = data_download,
                                   tmp_dir = tmp_dir,
                                   threshold = threshold,
                                   ... = ...)
...
#'
#' @description
#' The engine to download filesets from a site, using sequential computation.
#'
...
#' @return
#' @keywords internal
load_fileset.SEQengine = function(data_download, tmp_dir, threshold, ...){
  control = TRUE
  ...
  for(i in seq_len(length(list_DIR))){
    ...
    progressr::with_progress({
      ...
      download_success = purrr::map(.x = seq_len(nrow(download_attempt)),
